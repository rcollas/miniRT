...
_Bool	hit_cone_caps(t_ray *ray, t_obj *obj, t_ray *hit)
{
	...
	if (hit_disk(&ray_caps, &cone_caps, &hit_caps) && ray_caps.dist > 0.05
...
_Bool	solve_quadratic_cone(double *coeff, double *closest_hit)
{
	double	delta;
	double	roots[2];
...
_Bool	hit_cone(t_ray *ray, t_obj *obj, t_ray *hit)
{
	...
	if (solve_quadratic_cone(coeff, &ray->dist) && ray->dist > 0.05
