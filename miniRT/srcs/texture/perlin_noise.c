...
void	apply_perlin_noise(t_ray *hit)
{
	double	noise_coeff[3];
   
	noise_coeff[X] = (double)(noise3(0.1 * hit->origin.coord[X], 0.1 * hit->origin.coord[Y], 0.1 * hit->origin.coord[Z]));
	noise_coeff[Y] = (double)(noise3(0.1 * hit->origin.coord[Y], 0.1 * hit->origin.coord[Z], 0.1 * hit->origin.coord[X]));
	noise_coeff[Z] = (double)(noise3(0.1 * hit->origin.coord[Z], 0.1 * hit->origin.coord[X], 0.1 * hit->origin.coord[Y]));
	hit->dir.coord[X] = (1.0 - 0.2) * hit->dir.coord[X] + 0.2 * noise_coeff[X];
	hit->dir.coord[Y] = (1.0 - 0.2) * hit->dir.coord[Y] + 0.2 * noise_coeff[Y];
