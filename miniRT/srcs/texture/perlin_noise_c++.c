...
void	apply_bump_map(t_hit *hit)
{
	...
	noise_coeff[X] = noise(0.1 * hit->origin.coord[X], 0.1 * hit->origin.coord[Y], 0.1 * hit->origin.coord[Z]);
	noise_coeff[Y] = noise(0.1 * hit->origin.coord[Y], 0.1 * hit->origin.coord[Z], 0.1 * hit->origin.coord[X]);
	noise_coeff[Z] = noise(0.1 * hit->origin.coord[Z], 0.1 * hit->origin.coord[X], 0.1 * hit->origin.coord[Y]);
	hit->dir.coord[X] = hit->dir.coord[X] * noise_coeff[X];
	hit->dir.coord[Y] = hit->dir.coord[X] * noise_coeff[Y];
	hit->dir.coord[Z] = hit->dir.coord[X] * noise_coeff[Z];
	tmp = mul_vec3(hit->dir, hit->dir);
	if (!tmp)
		return ;
}
