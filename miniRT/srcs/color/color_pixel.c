...
t_rgb	get_ambient_light(t_scene *scene, t_hit hit)
{
	...
	ambient_light = mul_rgb(ambient_light, *scene->ambient_light->color);
	return (ambient_light);
}

t_rgb	get_diffuse_light(t_scene *scene, t_hit hit, t_vec3	light_dir)
{
	double	cos_theta;
	double	intensity;
	t_rgb	diffuse_light;
	...
	diffuse_light = mul_rgb_and_const(hit.color, intensity);
...
t_rgb	get_specular_light(t_scene *scene, t_hit hit, t_ray ray, t_vec3	light_dir)
{
...
t_rgb	sum_total_light(t_scene *scene, t_hit hit, t_ray ray, t_vec3 light_dir)
{
	t_rgb	total_light;

	total_light = init_color(0, 0, 0);
	total_light = add_rgb(total_light, get_ambient_light(scene, hit));
	total_light = add_rgb(total_light, get_diffuse_light(scene, hit, light_dir));
	total_light = add_rgb(total_light, get_specular_light(scene, hit, ray, light_dir));
	return (total_light);
}
...
void	get_color_pixel(t_scene *scene, t_hit hit, t_ray ray, int *color)
{
	...
	total_light = mul_rgb_and_const(total_light, hit.pixel_shadow);
